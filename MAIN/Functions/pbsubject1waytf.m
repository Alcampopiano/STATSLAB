...
if strcmp(options.FWE, 'benhoch')
    ...
    for i=1:conAcol;
        diffs{i,1}=zeros(STATS.nboot,STATS.timesout,STATS.freqbins);
    end
end
...
%         end
    ...
    for bandind=1:STATS.freqbins;
        ...
        for timecurrent=1:STATS.timesout;
            ...
            if strcmp(options.FWE, 'benhoch')
                for i=1:conAcol;
                    diffs{i,1}(:,timecurrent,bandind)=psihat(:,i);
                    %results.(field_name{filecurrent}).(band_fields{bandind}).factor_A.diffs{i,1}(:,timecurrent,bandind)=psihat(:,i);
                end
            end
    ...
    if strcmp(options.FWE, 'benhoch');
       [band_results] = FWEtime(results.(field_name{filecurrent}),STATS.alpha,STATS.nboot,'subjectTF',diffs);
